...
function net = model_init_HSI_SDeCNN
 
nch = 25; % number of considered channel in the denoising process
lr11  = [1 1];
lr10  = [1 0];
weightDecay = [1 1];
...
net.layers{end+1} = struct('type', 'conv', ...
    'weights', {{orthrize2(sqrt(2/(9*nCh))*randn(fSz,fSz,nCh,4,'single')), zeros(4,1,'single')}}, ...
    'stride', 1, ...
    'learningRate',lr10, ...
    'dilate',1, ...
    'weightDecay',weightDecay, ...
